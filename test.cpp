...
void eat(TokenType expected_type) {
	...
	if (expected_type != current_token.type) {
		std::cout << "Expected: " << expected_type << ", Received: " << current_token.type << "\n";
		parse_errors++;
	}
	consume();
}
void match_terminal(std::string expected) {
	if (current_token.type != Token::TERMINAL || current_token.lexeme != expected) {
		std::cout << "Expected: " << expected << ", Received " << current_token.lexeme << "\n";
		parse_errors++;
	}
	consume();
}
...
void parse() {
	program();
	std::cout << "There were " << parse_errors << " parse errors\n"
}